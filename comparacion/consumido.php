...
$query66= "SELECT granja.nombre as nn,sum(alimento_consumido.cantidad_h) as ah, sum(alimento_consumido.cantidad_m) as am from alimento_consumido, nave, lote, granja where alimento_consumido.id_nave = nave.id_nave AND nave.id_lote = lote.id_lote and lote.id_granja = granja.id_granja and granja.id_granja='".$rowgg['id_granja']."' AND (alimento_consumido.fecha_reg BETWEEN  '$fec_ini 00:00:00' AND  '$fecha_fin 23:59:59')  ORDER BY granja.id_granja";
